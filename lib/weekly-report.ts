...
[입력 형식]
{
  ...
  "previousReports": [ { "pageId", "title", "bodyStorage", "weekNumber" } ],
  "previousWeekNumber": 직전 주차 번호 | null
}

[비교 기준점 — 중요]
모든 "대비/변화" 분석의 기준점은 **직전 주차(previousWeekNumber)** 이다. 고정된 W2 가 아니다.
- previousWeekNumber = previousReports 중 weekNumber 가 가장 큰 항목 = 직전(가장 최근) 주차.
  (오프라인 주차는 스크럼 리포트가 없으므로, 직전 "슬랙 스크럼" 주차가 자동으로 비교 대상이 된다.)
- 이 직전 주차 리포트(bodyStorage)의 §1~§6 수치·명단을 baseline 으로 삼아 본 주차와 1:1 비교한다.
- 본문에 "W2" 를 비교 기준으로 임의로 박지 말 것. 반드시 previousWeekNumber 의 값을 "W{직전}" 형태로 표기.
- previousReports/previousWeekNumber 가 없으면 본 주차 단독 분석.
...
- 단계 변화 (L1→L3 같은 변동)
- Tier 변동
- "직전 주차(W{직전}) 대비 변화" 항목 최소 2개

[직전 주차 대비 분석 원칙]

본 리포트의 모든 "대비/변화"는 **직전 주차(previousWeekNumber, W{직전})** 를 기준점으로 한다.
W2 같은 특정 주차를 고정 기준으로 박지 말 것. previousReports(직전 주차 리포트 본문)가 있으면
다음을 반드시 직전 주차와 1:1 비교하여 출력한다.

1. §1 핵심 지표 — 각 행의 "메모" 컬럼에 직전 주차 대비 변화를 한 줄로 명시
   - 제출률: "본 주차 X% (n/N) | 직전 W{직전} Z% 대비 ↑/↓Y%pt" 같이
   - L2+ 진척률: "직전 W{직전} Z% → 본 주차 N% (Δ +/-)"
   - 엔노이아 활용 명시: 누적 N명 (직전 W{직전} 누적 X명 + 본 주차 신규 Y명)
   - 도움 요청: "본 주차 N건 (직전 주차 미해결 X건 + 신규 Y건)"

2. §3 진척 단계 분석 — 기수 교차표 아래에 변동 요약 추가
   - 직전 주차의 §3 단계 분류를 baseline 으로 비교.
   - "상승: X명 (예: 김민정 L1→L2)" / "정체: Y명" / "하락: Z명"
   - 핵심 변동 인물 2-3명 짧게 코멘트

3. §4 Tier 평가 — 각 Tier 표의 평가 포인트에 "직전 W{직전} Tier → 본 주차 Tier" 화살표
   (예: 이원희 A→S ↑) 와 변동 이유 한 줄. 직전 주차의 §4 Tier 를 baseline 으로 비교.
   - 본 주차 신규 Tier S/A 진입자는 별도 명시

4. §5 도움 요청 — 카테고리별 표에 "직전 주차 미해결" 마크 추가
   - 직전 주차 §5 와 비교해 "동일·유사 요청이 직전 주차에 있었는지" 컬럼 또는 메모로

5. §6 패턴 관찰 — "직전 주차 대비 변화" 항목 최소 2개 필수
   - 성장 궤적이 좋은 케이스 (Tier 상승, 단계 상승)
   - 정체·하락 케이스 (운영진 케어 필요 식별)
   - 누적 추세 (예: "엔노이아 활용 명시 직전 W{직전} 누적 P% → 본 주차 누적 N%")

직전 주차 데이터가 없으면 본 주차 단독 분석.

[엔노이아 활용 명시 누적 계산 룰 — §0·§1·§6 모두 적용]

엔노이아 명시는 한 번 명시하면 유지되는 **누적** 지표다. 단, baseline 은 직전 주차의 누적값이다.
"엔노이아 활용 명시자"는 ennoia/엔노이아/Ennoia 키워드를 스크럼 본문에 명시한 사람.

1. 직전 주차 리포트(previousReports 중 weekNumber 가 가장 큰 항목)의 §1 "엔노이아 활용 명시" 행에
   기록된 **직전 주차 누적 명단/명수** 를 baseline 으로 삼는다.
   (보강: 직전 주차 §2 표의 ✅/🎯/🚧 셀에 "엔노이아/ennoia/Ennoia" 가 있는 행의 이름도 누적 명단에 포함.)
2. 본 주차 participants[].submission.done/next/blocker 본문에서 같은 키워드가 등장하면
   해당 participant.name 을 명시자로 추가.
3. 직전 주차 누적 명단 ∪ 본 주차 신규 명단 (이름 중복 제거) = **본 주차 누적 명시자 명단**.
4. §1 핵심 지표 "엔노이아 활용 명시" 행:
   - 값 = "누적 N% (n/total)" 형식 (n = 누적 명단 크기, total = participants.length)
   - 메모 = "직전 W{직전} 누적 X명 + 본 주차 신규 Y명 = 누적 N명" 같이 변동 명시
   - 누적이므로 직전 주차 누적값 대비 절대 감소하지 않는다.
5. §0 요약과 §6 패턴 관찰에도 누적 비율 사용.

직전 주차 데이터가 없으면 본 주차 단독 카운트.

[미제출자 단계 처리 — 중요]

본 주차 미제출자를 이유 불문 L0 "시작 못함"으로 자동 분류하지 말 것. 미제출은 "이번 주에 응답이 없었다"는
뜻이지 "지금까지 아무것도 안 했다"는 뜻이 아니다.

1. previousReports 전체(제공된 모든 과거 리포트, 직전 1주만이 아님)의 §2 표에서 그 사람 이름을 찾아
   가장 최근에 실제 내용(done/next)이 채워져 있던 항목을 찾는다.
2. 찾았다면 §3 표에는 그 마지막 단계를 기재하고, 구성/메모에 "(W{그 주차} 이후 미제출)" 같이 표기한다.
   L0 칸에 넣지 말고 마지막으로 확인된 단계 칸에 넣는다.
3. previousReports 전체에서도 실제 제출 이력을 전혀 찾을 수 없는 사람만 진짜 L0 "시작 못함"으로 분류한다.
4. §0 요약, §6 패턴 관찰에서 이런 사람을 언급할 때도 "시작 못함"이 아니라 "N주 연속 미제출(마지막 확인:
   W{그 주차} L{단계})"처럼 사실대로 표현한다.
...
- 마지막 줄: _{scrumDate} 스크럼 응답을 바탕으로 작성했습니다._
- 마크다운만 사용. HTML 태그 X. JSON·코드 블록으로 감싸지 말 것.
- "다음과 같습니다", "이상입니다" 같은 메타 멘트 금지.

[톤 — AI 슬롭 회피, 중요]
사람이 직접 쓴 듯한 담백한 사내 보고서로 작성한다.
- 이모지는 §2 표 헤더(✅🎯🚧)와 진척 단계 화살표(↑↓→) 외에는 쓰지 않는다. 본문 narrative 에 이모지 금지.
- 감탄부호(!) 쓰지 않는다. 굵은 글씨(**)는 표의 이름·핵심 수치 등 꼭 필요한 곳에만, 남발 금지.
- 과장·상투적 평가어 자제: "뚜렷한 성장", "건강하게 다음 단계로", "약진", "두드러진다", "괄목할" 같은
  표현 대신 사실과 수치로 담백하게 기술한다. (예: "L2+ 가 67%에서 81%로 올랐다.")
...
export async function generateAndPublishWeeklyReport(
    ...
    for (const pid of ids) {
      try {
        ...
        previousReports.push({
          ...
          title: p.title,
          bodyStorage: p.body,
          weekNumber: parseWeekNumberFromTitle(p.title),
        });
      } catch (err) {
  ...
  // 직전 주차 = previousReports 중 weekNumber 가 가장 큰 것 (오프라인 주차는 리포트가 없어
  // 자동으로 직전 '슬랙 스크럼' 주차가 비교 대상이 됨). 본 주차보다 작은 것만 후보.
  const previousWeekNumber =
    previousReports
      .map((r) => r.weekNumber)
      .filter((w): w is number => typeof w === "number" && w < input.weekNumber)
      .sort((a, b) => b - a)[0] ?? null;
  ...
  const userPayload = JSON.stringify({
    ...
    // 이전 주차 리포트 (있을 때만). 직전 주차 대비 비교·누적 분석용 reference.
    previousReports,
    // 직전(가장 최근) 슬랙 스크럼 주차 번호. 모든 "대비" 비교의 기준점.
    previousWeekNumber,
  });
  ...
  // 3. Confluence 페이지 생성 (또는 existingPageId 지정 시 덮어쓰기)
...
/**
 * 리포트 페이지 제목에서 주차 번호 추출.
 * 생성 제목 형식: "4주차 실행 현황 (W4: 2026-06-04)".
 * "W4" 또는 "4주차" 어느 쪽이든 인식. 못 찾으면 undefined.
 */
export function parseWeekNumberFromTitle(title: string): number | undefined {
  if (!title) return undefined;
  const m = title.match(/W\s*(\d+)/i) ?? title.match(/(\d+)\s*주차/);
